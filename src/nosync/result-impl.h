...
{
    return std::get<std::optional<T>>(res_err) != std::nullopt;
}
...
{
    check_is_ok();
    return std::get<std::optional<T>>(res_err).value();
}
...
{
    check_is_ok();
    return std::move(std::get<std::optional<T>>(res_err).value());
}
...
template<typename T>
result<T>::result(std::tuple<std::optional<T>, std::error_code> res_err)
