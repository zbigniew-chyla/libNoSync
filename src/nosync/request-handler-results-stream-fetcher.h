...
keeps calling handle_request() on it using specified timeout and passes a result from each call
to results_handler function. The handle_request() calls are made sequentially, i.e. another call
is made after receiving a result from preceding one). Successive handle_request() call is made
only if results_handler for the previous one returns iteration_status::running.
