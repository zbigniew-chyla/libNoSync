// This file is part of libnosync library. See LICENSE file for license details.
#ifndef NOSYNC_UNIX__SOCKET_CONNECTIONS_BYTES_IO_ACCEPTOR_H
#define NOSYNC_UNIX__SOCKET_CONNECTIONS_BYTES_IO_ACCEPTOR_H

#include <functional>
#include <nosync/bytes-io.h>
#include <nosync/interface-type.h>
#include <nosync/ux/fd-watching-event-loop.h>
#include <nosync/ux/owned-fd.h>
#include <memory>


namespace nosync::ux
{

std::shared_ptr<interface_type> make_socket_connections_bytes_io_acceptor(
    fd_watching_event_loop &evloop, owned_fd &&listen_sock_fd,
    std::function<void(std::shared_ptr<bytes_io>)> &&io_handler,
    std::size_t read_buffer_size = 8192);

}

#endif /* NOSYNC_UNIX__SOCKET_CONNECTIONS_BYTES_IO_ACCEPTOR_H */
